...
namespace RefProp_wrapper
{
    public class iRefProp
    {
        ...
        //NIST is no longer documenting them. Also please note the manual states that legacy subroutines could
        //be dropped in some future release of this software. 
        ...
        public static int DLLinit()
        {
            ...
            // if no RefProp environment variable, see if RefProp located in its default location
            if (pDLL == IntPtr.Zero)
            {
            ...
            if (pDLL == IntPtr.Zero)
                ...
                // use the Win32 SearchPath function to locate DLL
                if (SearchPath(null, RefPropFileName, null, WIN32MAXPATH, rppath,
                    out filepart) > 0)
                {
        ...
        public bool NISTSetup(string FluidName)
        {
            ...
            int i, sc, ncc, nc, nc2 = 0, ierr = 0, mass = 0, ixflag = 2,
                ...
                hFiles_length = 10000, herr_length = 255;
            double sum, hRef = 0.0, sRef = 0.0, Tref = 0.0, pref = 0.0;
            double[] xkg = new double[MaxComps], ztemp = new double[MaxComps];
            string hFmix, hrf, hMixNme, hFiles, a, ab, FluidNme, FlNme, refpropEnv, FluidsPrefix, MixturesPrefix, herr;
            FlNme = FluidName;
            refpropEnv = Environment.GetEnvironmentVariable("RPPrefix");
            if (!String.IsNullOrEmpty(refpropEnv))
            {
                FluidsPrefix = refpropEnv + FluidsDirectory;
                MixturesPrefix = refpropEnv + MixturesDirectory;
            }
            else
            {
                FluidsPrefix = RefPropDirectory + FluidsDirectory;
                MixturesPrefix = RefPropDirectory + MixturesDirectory;
            }
            hrf = "DEF";
        ...
        private static void CheckName(ref string FluidName)
        {
            char[] quote = new char[1] { '\"' };
            FluidName = FluidName.Trim(quote);
            string FluidNameUC = FluidName.ToUpper();
            if (FluidNameUC == "AIR") FluidName = "nitrogen;7812;argon;0092;oxygen;2096";
            else if (FluidNameUC == "CARBON DIOXIDE") FluidName = "CO2";
            else if (FluidNameUC == "CARBON MONOXIDE") FluidName = "CO";
            else if (FluidNameUC == "CARBONYL SULFIDE") FluidName = "COS";
            else if (FluidNameUC == "CYCLOHEXANE") FluidName = "CYCLOHEX";
            else if (FluidNameUC == "CYCLOPROPANE") FluidName = "CYCLOPRO";
            else if (FluidNameUC == "DEUTERIUM") FluidName = "D2";
            else if (FluidNameUC == "HEAVY WATER") FluidName = "D2O";
            else if (FluidNameUC == "HYDROGEN SULFIDE") FluidName = "H2S";
            else if (FluidNameUC == "IBUTANE") FluidName = "ISOBUTAN";
            else if (FluidNameUC == "ISOBUTANE") FluidName = "ISOBUTAN";
            else if (FluidNameUC == "ISOPENTANE") FluidName = "IPENTANE";
            else if (FluidNameUC == "NEOPENTANE") FluidName = "NEOPENTN";
            else if (FluidNameUC == "ISOHEXANE") FluidName = "IHEXANE";
            else if (FluidNameUC == "NITROUS OXIDE") FluidName = "N2O";
            else if (FluidNameUC == "PARAHYDROGEN") FluidName = "PARAHYD";
            else if (FluidNameUC == "PROPYLENE") FluidName = "PROPYLEN";
            else if (FluidNameUC == "SULFUR HEXAFLUORIDE") FluidName = "SF6";
        }
        ...
        // dllverrefprop - returns RefProp DLL version using refprop2dll
        public static string dllverrefprop()
        {
            int ierr = 0, iFlag, iUnits, hFld_length, hIn_length,
                hOut_length, herr_length = 255;
            double t = 0, d = 0, q = 0.0;
            double[] z1 = new double[MaxComps], Output = new double[MaxOutput];
            string FluidName = " ", hIn = " ", hOut = "dll#",
                herr = new string(' ', herr_length);
            iFlag = 0; //do not call SATSPLN() subroutine
            iUnits = 5; //English units=5
            hFld_length = FluidName.Length;
            hIn_length = hIn.Length;
            hOut_length = hOut.Length;
            refprop2dll(FluidName, hIn, hOut, ref iUnits, ref iFlag, ref t, ref d, z1, Output, ref q,
                ref ierr, ref herr, ref hFld_length, ref hIn_length, ref hOut_length, ref herr_length);
            return herr;
        }

        // sattdpt - Calculated saturated thermodynamic/thermophysical properties given temperature (K)
        // return values:
        //    true = success
        ...
        // sattdpp - Calculated saturated thermodynamic/thermophysical properties given pressure (kPa)
        // return values:
        //    true = success
        //    false = failure.  See herrstr for error message
        ...
        // flshtdp - Calculated thermodynamic/thermophysical properties given temperature (K) and pressure (kPa)
        // return values:
        //    true = success
        //    false = failure.  See herrstr for error message
        ...
        // refpropt - Calculated saturated thermodynamic/thermophysical properties given temperature (K)
        // Does not require a call to NISTSetup()
        // return values:
        //    true = success
        //    false = failure.  See herrstr for error message
        public bool refpropt(string FluidName, double t, int kph, ref double p, ref double density, ref double h,
            ref double s, ref double cv, ref double cp, ref double w, ref double eta, ref double tcx)
        {
            bool retval = false;
            int ierr = 0, iMass, iFlag, iUnits, iUCode = 0, hFld_length,
                hIn_length, hOut_length, hUnits_length = 255, herr_length = 255;
            double q;
            double[] xliq = new double[MaxComps], yvap = new double[MaxComps],
                Output = new double[MaxOutput], x3 = new double[MaxComps];
            string hIn = "Tq", hOut = "P,D,h,s,Cv,Cp,w,vis,tcx,tc,pc,dc,m",
                hUnits = new string(' ', hUnits_length),
                herr = new string(' ', herr_length);
            iMass = 1; //use mass units, not molar units
            iFlag = 1; //call SATSPLN() subroutine
            iUnits = 5; //English units=5
            hFld_length = FluidName.Length;
            hIn_length = hIn.Length;
            hOut_length = hOut.Length;
            q = (kph == BUBBLEPOINT) ? 0.0 : 1.0; //0=saturated liquid, 1=saturated vapor
            if (refpropdll != null)
            {
                refpropdll(FluidName, hIn, hOut, ref iUnits, ref iMass, ref iFlag, ref t, ref q, z, Output,
                    ref hUnits, ref iUCode, xliq, yvap, x3, ref q, ref ierr, ref herr, ref hFld_length,
                    ref hIn_length, ref hOut_length, ref hUnits_length, ref herr_length);
                if (ierr == 0)
                {
                    p = Output[0];
                    density = Output[1];
                ...
                herrstr = "refpropdll() subroutine not found";
            return retval;
        }

        // refpropp - Calculated saturated thermodynamic/thermophysical properties given pressure (kPa)
        // Does not require a call to NISTSetup()
        // return values:
        //    true = success
        //    false = failure.  See herrstr for error message
        public bool refpropp(string FluidName, double p, int kph, ref double t, ref double density, ref double h,
            ref double s, ref double cv, ref double cp, ref double w, ref double eta, ref double tcx)
        {
            bool retval = false;
            int ierr = 0, iMass, iFlag, iUnits, iUCode = 0, hFld_length,
            ...
            double[] xliq = new double[MaxComps], yvap = new double[MaxComps],
                Output = new double[MaxOutput], x3 = new double[MaxComps];
            string hIn = "Pq", hOut = "T,D,h,s,Cv,Cp,w,vis,tcx,tc,pc,dc,m",
                hUnits = new string(' ', hUnits_length),
                herr = new string(' ', herr_length);
            iMass = 1; //use mass units, not molar units
            iFlag = 1; //call SATSPLN() subroutine
            iUnits = 5; //English units
            hFld_length = FluidName.Length;
            hIn_length = hIn.Length;
            hOut_length = hOut.Length;
            q = (kph == BUBBLEPOINT) ? 0.0 : 1.0; //0=saturated liquid, 1=saturated vapor
            if (refpropdll != null)
            {
                refpropdll(FluidName, hIn, hOut, ref iUnits, ref iMass, ref iFlag, ref p, ref q, z, Output,
                    ref hUnits, ref iUCode, xliq, yvap, x3, ref q, ref ierr, ref herr, ref hFld_length,
                    ref hIn_length, ref hOut_length, ref hUnits_length, ref herr_length);
                if (ierr == 0)
                {
        ...
        // refproppt - Calculated thermodynamic/thermophysical properties given temperature (K) and pressure (kPa)
        // Does not require a call to NISTSetup()
        // return values:
        //    true = success
        //    false = failure.  See herrstr for error message
        public bool refproppt(string FluidName, double t, double p, ref double density, ref double h,
            ref double q, ref double s, ref double cv, ref double cp, ref double w, ref double eta,
            ref double tcx)
        {
            bool retval = false;
            int ierr = 0, iMass, iFlag, iUnits, iUCode = 0, hFld_length,
            ...
            double[] xliq = new double[MaxComps], yvap = new double[MaxComps],
                Output = new double[MaxOutput], x3 = new double[MaxComps];
            string hIn = "PT", hOut = "D,h,s,Cv,Cp,w,vis,tcx,tc,pc,dc,m",
                hUnits = new string(' ', hUnits_length),
                herr = new string(' ', herr_length);
            iMass = 1; //use mass units, not molar units
            iFlag = 1; //call SATSPLN() subroutine
            ...
            if (refpropdll != null)
            {
                refpropdll(FluidName, hIn, hOut, ref iUnits, ref iMass, ref iFlag, ref p, ref t, z, Output,
                    ref hUnits, ref iUCode, xliq, yvap, x3, ref q, ref ierr, ref herr, ref hFld_length,
                    ref hIn_length, ref hOut_length, ref hUnits_length, ref herr_length);
                if (ierr == 0 || ierr == -810)
                { // if in the two-phase region, REFPROP gives -810 warning stating certain properties cannot be calculated.
                  //     Check "q" to determine if you are in the two-phase region
                    density = Output[0];
                    h = Output[1];
                    s = Output[2];
                    cv = Output[3];
